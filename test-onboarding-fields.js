...
const companiesResponse = await makeRequest(options);
    ...
    if (companiesResponse.status !== 200) {
      console.error('❌ Failed to fetch companies:', companiesResponse.status, companiesResponse.data);
      return;
    }
    ...
    console.log(`   - Companies with onboarding_status: ${companies.filter(c => c.properties?.onboarding_status).length}`);
    console.log(`   - Companies with onboarding_step: ${companies.filter(c => c.properties?.onboarding_step).length}`);
    
    if (!hasOnboardingStatus && !hasOnboardingStep) {
      console.log('\n⚠️  Note: No companies have onboarding fields populated yet.');
      console.log('   This is expected if the custom properties haven\'t been created in HubSpot yet.');
