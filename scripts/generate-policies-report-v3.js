...
async function getTransferPolicies(transferId) {
  try {
    ...
    // First, try to get transfer details from your local database
    // This would normally query your ACHTransaction table
    const transfer = await getTransferFromDatabase(transferId)
    if (!transfer) {
      return { transferId, error: "Transfer not found in local database" }
    }

    // Now search for customer in HubSpot using company name or customer name
    let customerData = null
    
    // Try searching by company name first
    if (transfer.companyName) {
      customerData = await searchCustomerByName(transfer.companyName)
    }
    
    // If no company found, try by customer name
    if (!customerData && transfer.customerName) {
      customerData = await searchCustomerByName(transfer.customerName)
    }

    if (!customerData) {
      return { transferId, error: "Customer not found in HubSpot", transfer }
    }

    // Get SOB and policies
    const sobData = await getSOBAndPolicies(customerData.company.id)
    if (!sobData) {
      return { transferId, error: "No SOB found for customer", transfer, customer: customerData.company }
    }
...
async function generatePoliciesReport() {
  ...
  const filename = `policies-report-v3-${new Date().toISOString().split('T')[0]}.json`
  fs.writeFileSync(filename, JSON.stringify(reportData, null, 2))
  console.log(`\n💾 Detailed report saved to: ${filename}`)

  // Generate CSV summary
  const csvFilename = `policies-summary-v3-${new Date().toISOString().split('T')[0]}.csv`
