...
async function searchAllCompanies() {
  console.log("🔍 Searching for all companies in HubSpot...")
  
  const options = {
    hostname: "api.hubapi.com",
    path: `/crm/v3/objects/companies?properties=name,dwolla_customer_id&limit=100`,
    method: "GET",
    headers: {
...
async function generatePoliciesReport() {
  console.log("🔍 Generating Comprehensive Policies Report - Direct HubSpot Query")
  console.log(`📊 Processing ${DWOLLA_TRANSFER_IDS.length} Dwolla Transfer IDs...\n`)

  // First, get all companies from HubSpot
  const companies = await searchAllCompanies()
  
  // Create a mapping of Dwolla IDs to companies
  const dwollaToCompanyMap = new Map()
  companies.forEach(company => {
    if (company.properties.dwolla_customer_id) {
      dwollaToCompanyMap.set(company.properties.dwolla_customer_id, company)
    }
  })

  console.log(`📊 Found ${dwollaToCompanyMap.size} companies with Dwolla IDs`)
  ...
  for (let i = 0; i < DWOLLA_TRANSFER_IDS.length; i++) {
    ...
    console.log(`🔍 Processing transfer ${i + 1}/${DWOLLA_TRANSFER_IDS.length}: ${transferId}`)

    const company = dwollaToCompanyMap.get(transferId)
    if (!company) {
    ...
    const sobData = await getCompanySOBs(company.id)
    if (!sobData) {
      errors.push({ transferId, error: "No SOB found for company", company: company.properties.name })
      continue
    }
  ...
  const filename = `policies-report-direct-${new Date().toISOString().split('T')[0]}.json`
  fs.writeFileSync(filename, JSON.stringify(reportData, null, 2))
  console.log(`\n💾 Detailed report saved to: ${filename}`)

  // Generate CSV summary
  const csvFilename = `policies-summary-direct-${new Date().toISOString().split('T')[0]}.csv`
