...
function checkComponentFieldUsage() {
  ...
  for (const [componentName, config] of Object.entries(uiComponentFields)) {
    ...
    for (const [field, description] of Object.entries(config.fields)) {
      ...
      const isFound = fieldPatterns.some(pattern => 
        fileContent.includes(pattern) || 
        fileContent.includes(pattern.replace('.', '?.'))
      );
    ...
    console.log(`  ✅ Found ${foundFields.length}/${Object.keys(config.fields).length} expected fields`);
