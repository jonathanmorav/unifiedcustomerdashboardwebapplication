...
async function checkAssociationSchemas() {
  ...
  try {
    ...
    if (response.status === 200) {
      ...
      const sobAssociations = response.data.results.filter(assoc => 
        assoc.fromObjectTypeId === 'companies' || assoc.toObjectTypeId === 'companies'
      );
...
async function testCompanyAssociation(companyId) {
  ...
  const approaches = [
    { name: 'summary_of_benefits', path: `/crm/v3/objects/companies/${companyId}/associations/summary_of_benefits` },
    { name: '2-134825309', path: `/crm/v3/objects/companies/${companyId}/associations/2-134825309` },
