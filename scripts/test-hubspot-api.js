...
async function testHubSpotAPI() {
  ...
  if (HUBSPOT_API_KEY.includes('[REPLACE')) {
    ...
    console.error('Please replace [REPLACE_WITH_YOUR_HUBSPOT_API_KEY] with your actual HubSpot API key');
    process.exit(1);
  }
      ...
      res.on('end', () => {
        if (res.statusCode === 200) {
          try {
            ...
            const customObjects = response.results?.filter(schema => 
              !['companies', 'contacts', 'deals', 'tickets'].includes(schema.name)
            ) || [];
            ...
            const sobObject = customObjects.find(obj => 
