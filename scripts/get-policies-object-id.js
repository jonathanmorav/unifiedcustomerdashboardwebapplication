...
async function getPoliciesObjectInfo() {
  ...
  try {
    ...
    if (response.status === 200) {
      ...
      const customObjects = response.data.results.filter(
        (schema) =>
          ![
            "companies",
            "contacts",
      ...
      const policiesObject = response.data.results.find(
        (schema) =>
      ...
      if (policiesObject) {
        ...
        console.log(
          `   Labels: ${policiesObject.labels?.singular} / ${policiesObject.labels?.plural}`
        )
