...
async function getCompanyProperties() {
  ...
  try {
    ...
    if (response.status === 200) {
      ...
      const emailProperties = response.data.results.filter(prop => 
        prop.name.toLowerCase().includes('email') ||
        prop.label.toLowerCase().includes('email')
      );
      ...
      const ownerProperties = response.data.results.filter(prop => 
        prop.name.toLowerCase().includes('owner') ||
        prop.label.toLowerCase().includes('owner')
      );
