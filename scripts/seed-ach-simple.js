...
const statuses = ["pending", "processing", "processed", "failed", "cancelled", "returned"]
...
async function generateMockTransactions(count = 100) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const transaction = {
      ...
      created: created,
      sourceName:
        direction === "debit" ? names[Math.floor(Math.random() * names.length)] : "Company Account",
      destinationName:
        direction === "credit"
      ...
      fees: status === "processed" ? parseFloat((amount * 0.0029).toFixed(2)) : 0,
      netAmount:
        status === "processed" ? amount - parseFloat((amount * 0.0029).toFixed(2)) : amount,
      clearingDate: status === "processed" ? new Date(created.getTime() + 86400000 * 3) : null,
      processedAt: status === "processed" ? new Date(created) : null,
      metadata: {
        source: "demo",
