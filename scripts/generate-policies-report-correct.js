...
// Your complete list of Dwolla Transfer IDs (which are actually HubSpot Company IDs)
...
async function generatePoliciesReport() {
  console.log("🔍 Generating Comprehensive Policies Report - CORRECTED APPROACH")
  ...
  for (let i = 0; i < DWOLLA_TRANSFER_IDS.length; i++) {
    ...
    console.log(`\n🔍 Processing transfer ${i + 1}/${DWOLLA_TRANSFER_IDS.length}: ${transferId}`)
    ...
    try {
      ...
      console.log(`✅ Found company: ${company.properties.name || 'Unnamed'}`)
      ...
      console.log(`  📋 SOBs: ${sobData.sobs.length}`)
      console.log(`  🏛️ Policies: ${sobData.totalPolicies}`)
      console.log(`  🚨 Double Bill: ${hasDoubleBill ? 'YES' : 'NO'}`)
  ...
  const filename = `policies-report-correct-${new Date().toISOString().split('T')[0]}.json`
  fs.writeFileSync(filename, JSON.stringify(reportData, null, 2))
  console.log(`\n💾 Detailed report saved to: ${filename}`)

  // Generate CSV summary
  const csvFilename = `policies-summary-correct-${new Date().toISOString().split('T')[0]}.csv`
