...
function verifyHubSpotMappings() {
  ...
  const companyMappings = [
    ...
    {
      api: company.properties.owner_email,
      ui: "hubspot.company.ownerEmail",
      expected: "support@acme.com",
    },
    {
      api: company.properties.dwolla_customer_id,
      ui: "hubspot.company.dwollaId",
      expected: "e8b0f3d2-4a89-4c6b-8383-1234567890ab",
    },
  ]
  ...
  companyMappings.forEach((mapping) => {
    ...
    log(
      `  ${status} ${mapping.ui}: ${mapping.api}`,
      mapping.api === mapping.expected ? "green" : "red"
    )
  })
  ...
  const sobMappings = [
    {
      api: sob.properties.amount_to_draft,
      ui: "hubspot.summaryOfBenefits.amountToDraft",
      expected: 2500.0,
    },
    { api: sob.properties.fee_amount, ui: "hubspot.summaryOfBenefits.feeAmount", expected: 125.0 },
    {
      api: sob.properties.pdf_document_url,
      ui: "hubspot.summaryOfBenefits.pdfDocumentUrl",
      expected: "https://example.com/sob/2025-01.pdf",
    },
  ]
  ...
  const policyMappings = [
    ...
    {
      api: policy.properties.policy_holder_name,
      ui: "policy.policyHolderName",
...
function verifyDwollaMappings() {
  ...
  const customerMappings = [
    ...
    {
      api: `${customer.firstName} ${customer.lastName}`,
      ui: "dwolla.customer.name",
      expected: "John Doe",
    },
    { api: customer.status, ui: "dwolla.customer.status", expected: "verified" },
    {
      api: customer.businessName,
      ui: "dwolla.customer.businessName",
      expected: "Acme Corporation",
    },
  ]
  ...
  const sourceMappings = [
    ...
    {
      api: source.accountNumberMasked,
      ui: "dwolla.fundingSource.accountNumberMasked",
      expected: "****1234",
    },
