...
async function checkContactProperties() {
  try {
    ...
    const clarityProps = response.data.results.filter(
      (prop) =>
  ...
  try {
    ...
    if (clarityMentions.length > 0) {
      ...
      clarityMentions.slice(0, 3).forEach((engagement, index) => {
        console.log(
          `  ${index + 1}. ${engagement.properties?.hs_engagement_type} - ${engagement.properties?.hs_body_preview?.substring(0, 100)}...`
        )
      })
    }
  } catch (error) {
