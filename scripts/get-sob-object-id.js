...
async function getSOBObjectInfo() {
  ...
  try {
    ...
    if (response.status === 200) {
      const sobObject = response.data.results.find(schema => 
      ...
      if (sobObject) {
        ...
        response.data.results
          ...
          .forEach(schema => {
            console.log(`   - ${schema.name} (${schema.labels?.singular}) - ID: ${schema.objectTypeId || schema.id}`);
          });
      }
    } else {
  ...
  try {
    ...
    if (response.status === 200) {
      ...
      if (response.data.results?.length > 0) {
        console.log('   Sample association IDs:', response.data.results.slice(0, 3).map(r => r.id));
        return objectTypeId;
      }
