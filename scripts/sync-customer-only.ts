          ...
          if (customerEmail) {
            ...
            await prisma.aCHTransaction.create({
              data: {
                ...
                direction: destUrl.includes(process.env.DWOLLA_MASTER_ACCOUNT_ID) ? 'credit' : 'debit',
                created: new Date(transfer.created),
                sourceId: sourceUrl.split('/').pop(),
                sourceName: sourceUrl.includes('/customers/') ? customerName : 'Cakewalk Benefits Inc.',
                destinationId: destUrl.split('/').pop(),
                destinationName: destUrl.includes('/customers/') ? customerName : 'Cakewalk Benefits Inc.',
