...
const REQUIRED_VARS = [
  ...
  },
  {
    ...
    validate: (v) => v.startsWith("GOCSPX-"),
    error: "Invalid Google Client Secret format",
  },
  {
    name: "DATABASE_URL",
    validate: (v) => v.includes("sslmode=require"),
    error: "Must include sslmode=require for production",
  },
  {
    name: "AUTHORIZED_EMAILS",
    validate: (v) => v.includes("@"),
    error: "Must contain valid email addresses",
  },
  {
    name: "HUBSPOT_API_KEY",
    validate: (v) => v.startsWith("pat-"),
    error: "Must be a valid HubSpot private app key",
  },
...
function loadEnv() {
  ...
  if (!fs.existsSync(envPath)) {
    ...
    console.log(
      `${colors.cyan}ℹ️  Looking for environment variables in current process...${colors.reset}\n`
    )
    return false
  }
...
function validateConfig() {
  ...
  RECOMMENDED_VARS.forEach((name) => {
    ...
    if (!value || value.trim() === "") {
      console.log(
        `${colors.yellow}⚠️  ${name}: Not set (recommended for production)${colors.reset}`
      )
      hasWarnings = true
    } else {
  ...
  if (hasErrors) {
    ...
    console.log(
      `${colors.yellow}Consider addressing the warnings for optimal security.${colors.reset}`
    )
  } else {
    console.log(`${colors.green}✅ Configuration validation PASSED${colors.reset}`)
    console.log(
      `${colors.green}Your configuration is ready for production deployment!${colors.reset}`
    )
  }
...
if (require.main === module) {
  ...
  if (!envLoaded) {
    console.log(
      `${colors.cyan}Tip: Create .env.production.local from .env.production template${colors.reset}\n`
    )
  }
