  ...
  try {
    ...
    const rateLimitResult = await rateLimiter.limit(request, {
      ...
      onLimitReached: async () => {
        await AccountSecurity.escalateSecurityEvent(session.user.id, "MFA_DISABLE_RATE_LIMITED", {
          ipAddress,
    ...
    const mfaResult = await MFAService.verifyTOTP(session.user.id, currentCode, ipAddress)
    ...
    // Disable MFA
    await MFAService.disableMFA(targetId, reason, isSelfService ? undefined : session.user.id)
