...
export class MetricsCollector {
  ...
  private initializeMetrics() {
    ...
    this.createCounter(
      ...
      'Total number of HubSpot API calls'
    )
    
    this.createCounter(
      'dwolla_api_calls_total',
      'Total number of Dwolla API calls'
    )
   ...
   */
  createHistogram(name: string, help: string, buckets: number[] = MetricsCollector.DEFAULT_BUCKETS) {
    ...
    for (const histogram of this.histograms.values()) {
      ...
      for (const [labelKey, value] of histogram.values) {
        ...
        for (const [bucket, count] of value.buckets) {
          lines.push(`${histogram.name}_bucket{le="${bucket}"${labels ? ',' + labels : ''}} ${count}`)
        }
        lines.push(`${histogram.name}_bucket{le="+Inf"${labels ? ',' + labels : ''}} ${value.count}`)
