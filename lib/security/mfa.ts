...
export class MFAService {
  ...
  static async verifyBackupCode(
    ...
    const codeIndex = user.mfaBackupCodes.findIndex((hash) =>
      Encryption.verifyBackupCode(code, hash)
    )
   ...
   */
  static async disableMFA(userId: string, reason: string, performedBy?: string): Promise<void> {
    await prisma.user.update({
      where: { id: userId },
      data: {
        mfaEnabled: false,
